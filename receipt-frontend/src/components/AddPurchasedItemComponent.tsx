...
const AddPurchasedItemComponent: FC<AddPurchasedItemComponentProps> = ({
  onChange
}) => {
  ...
  return (
    <>
      ...
      {purchasedItems.map((item, index) => (
        <PurchasedItem key={`${item.price}-${item.shortDescription}-${index}`} receiptItem={item} index={index + 1}/>
      ))}
